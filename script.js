...
function carInfo(inventory,id){
    ...
    if(car){
        return `Car ${id} is a ${car.car_year} ${car.car_model}`;
    }
}
...
function sortedCarModel(inventory){
    ...
    let sorted_car=inventory.map((carModel)=>{
...
function carYear(inventory){
    let only_car_year=inventory.map((year)=>{
        return year.car_year
    })
    return only_car_year
}

function olderThan2000(inventory){

    let new_car_collection=[];
    inventory.forEach((val)=>{
        if(val.car_year<2000){
            new_car_collection.push(val.car_year);
        }
    });
    return `There are total ${new_car_collection.length} cars and cars are: ${new_car_collection}`
}

function bmwAndaudi(inventory){

    let bmwandaudi=inventory.filter((val)=>{
        return val.car_make==="Audi" || val.car_make==="BMW"
    })
    return bmwandaudi
}
...
let id=Number(prompt("Enter the id between 1 and 50"));
console.log(carInfo(inventory,id));
console.log(lastCar(inventory));
console.log(sortedCarModel(inventory));
console.log(carYear(inventory));
console.log(olderThan2000(inventory));
const bmwAndAudiarray=(bmwAndaudi(inventory));
